...
export async function assertPropType(notion, dbId, name, type) {
    const db = await notion.databases.retrieve({ database_id: dbId });
    const prop = db.properties[name];
    if (!prop)
        throw new Error(`Property "${name}" not found.`);
    if (prop.type !== type)
        throw new Error(`Property "${name}" must be ${type}, got ${prop.type}.`);
}
export async function validateDatabaseSchema(notion, databaseId) {
