...
async function fixRichTextReviewTarget() {
  try {
    console.log('🔧 Fixing Rich Text Review Target values...\n');
    ...
    for (const page of response.results) {
      ...
      if (currentTargetValue !== expectedTarget) {
        console.log(`   🔧 Fixing Rich Text field...`);
        ...
        try {
          ...
          // 如果是 select 字段，尝试 select 格式
          if (error.message.includes('select')) {
            console.log(`   🔄 Trying select format...`);
            try {
              await notion.pages.update({
                page_id: page.id,
                properties: {
              ...
              console.log(`   ✅ Fixed with select format: "${expectedTarget}"`);
              fixedCount++;
            } catch (selectError) {
              console.log(`   ❌ Select format also failed: ${selectError.message}`);
            }
          }
        }
      } else {
